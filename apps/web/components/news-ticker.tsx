...
export function NewsTicker() {
    ...
    fetch("/api/news")
      ...
      .then((data) => {
        if (data.items?.length) {
          // İlk 15 haberi al
          setNews(
            data.items.slice(0, 15).map((item: NewsItem) => ({
              title: item.title,
              link: item.link,
              source: item.source,
            }))
  ...
  // Haberleri birleştir ve kayan yazıya çevir
      ...
      <div className="flex items-center">
        ...
        <div className="relative flex-1 overflow-hidden py-2">
          <div className="ticker-track flex whitespace-nowrap">
            <span className="ticker-content inline-block px-4 text-sm font-medium text-green-800 dark:text-green-200">
              {tickerText}
            </span>
            <span className="ticker-content inline-block px-4 text-sm font-medium text-green-800 dark:text-green-200">
              {tickerText}
